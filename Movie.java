...
public class Movie extends Item implements Serializable
{
	...
	{	
		return String.format("Movie: Title: %20s, Director: %20s, Genre: %10s, Status: %15s",
				title.substring(0, Math.min(title.length(), 20)), director.substring(0, Math.min(author.length(), 20)), genre.substring(0,  Math.min(genre.length(),  10)), chkout); 
	}
}
